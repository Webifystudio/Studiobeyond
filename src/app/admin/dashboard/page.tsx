...
import { BookOpen, Image as ImageIcon, Users, FileText, Eye, BarChart3, Layers, ListChecks, Newspaper } from "lucide-react";
...
async function getStatsData(): Promise<DashboardStats> {
  try {
    const [
      ...
      getDocs(collection(db, "newsItems")),
    ]);
    ...
    return {
      ...
      newsItems: newsItemsSnap.size,
      totalViews: totalViews,
      totalChapters: totalChapters,
    };
  } catch (error) {
    console.error("Error fetching dashboard stats:", error);
    return { categories: 0, mangas: 0, sliderItems: 0, customPages: 0, users: 0, newsItems: 0, totalViews: 0, totalChapters: 0 };
  }
}
...
export default function AdminDashboardPage() {
  ...
  const statCards: StatItem[] = [
    { title: "Total Categories", value: statsData.categories.toString(), icon: ListChecks, href: "/admin/dashboard/categories", description: "Manage manga categories.", color: "text-cyan-400" },
    ...
    { title: "News Items", value: statsData.newsItems.toString(), icon: Newspaper, href: "/admin/dashboard/news", description: "Manage news & updates.", color: "text-orange-400" },
    ...
    { title: "Total Chapters", value: statsData.totalChapters.toString(), icon: Layers, href: "/admin/dashboard/pages", description: "Across all custom pages.", color: "text-indigo-400" },
  ];

  const chartData = [
    { item: "Categories", count: statsData.categories },
    { item: "Mangas", count: statsData.mangas },
    { item: "News", count: statsData.newsItems },
    ...
    <div className="space-y-8">
      ...
      <Card className="bg-neutral-medium border-neutral-light">
        ...
        <CardContent className="text-neutral-extralight/80 space-y-2">
          <p>1. <Link href="/admin/dashboard/categories" className="text-brand-primary hover:underline">Manage Categories</Link>: Add or edit manga categories.</p>
          <p>2. <Link href="/admin/dashboard/mangas" className="text-brand-primary hover:underline">Manage Mangas</Link>: Add new manga series and assign categories.</p>
          <p>3. <Link href="/admin/dashboard/news" className="text-brand-primary hover:underline">Manage News</Link>: Post updates and announcements.</p>
          <p>4. <Link href="/admin/dashboard/slider" className="text-brand-primary hover:underline">Manage Slider</Link>: Update the hero section on the homepage.</p>
          <p>5. <Link href="/admin/dashboard/pages" className="text-brand-primary hover:underline">Manage Pages</Link>: Create and configure custom content pages.</p>
